...
do
  ...
  do
    ...
    /usr/bin/time -a -o ${resultPath}/time.csv -f "%E,%P,%S,%U,%e" /tmp/${algorithm} --image_before=${dataPath}/${imageBefore} --image_after=${dataPath}/${imageAfter} --output_flow=${resultPath}/flow/${resultCounter}.flo --pyr_scale=${2:-0.5} --levels=${3:-3} --winsize=${4:-15} --iterations=${5:-3} --poly_n=${6:-5} --poly_sigma=${7:-1.2} 
