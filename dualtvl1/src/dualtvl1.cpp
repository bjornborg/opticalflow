...
int32_t main(int32_t argc, char **argv) {
  ...
  if ((0 == commandlineArguments.count("image_before")) ||
    ...
    std::cerr << argv[0]
              << " estimates the optic flow using dualtvl1 algorithm ."
              << std::endl;
    std::cerr
        << "Usage:   " << argv[0] << " --image_before=<first image>"
        ...
        << " --epsilon=<Stopping criterion threshold used in the numerical "
           "scheme>\n"
        << " --innnerIterations=<Stopping criterion iterations number used in "
           "the numerical schem>\n"
        << " --outerIterations=<Stopping criterion iterations number used in "
           "the numerical schem>\n"
        << " --scaleStep=<Stepping size for pyramid scheme>\n"
        << " --gamma=<coefficient for additional illumination variation term>\n"
        << " --medianFiltering=<Median filter kernel size (1 = no filter) (3 "
           "or 5)>\n"
        << std::endl;
  ...
  int32_t const innnerIterations =
      std::stoi(commandlineArguments["innnerIterations"]);
  int32_t const outerIterations =
      std::stoi(commandlineArguments["outerIterations"]);
  double const scaleStep = std::stod(commandlineArguments["scaleStep"]);
  double const gamma = std::stod(commandlineArguments["gamma"]);
  int32_t const medianFiltering =
      std::stoi(commandlineArguments["medianFiltering"]);
  bool const useInitialFlow = false;

  // tau Time step of the numerical scheme.
  // lambda Weight parameter for the data term, attachment parameter. This is
  // the most relevant parameter, which determines the smoothness of the output.
  // The smaller this parameter is, the smoother the solutions we obtain. It
  // depends on the range of motions of the images, so its value should be
  // adapted to each image sequence. theta Weight parameter for (u - v)\^2,
  // tightness parameter. It serves as a link between the attachment and the
  // regularization terms. In theory, it should have a small value in order to
  // maintain both parts in correspondence. The method is stable for a large
  // range of values of this parameter. nscales Number of scales used to create
  // the pyramid of images. warps Number of warpings per scale. Represents the
  // number of times that I1(x+u0) and grad( I1(x+u0) ) are computed per scale.
  // This is a parameter that assures the stability of the method. It also
  ...
  // a trade-off between precision and running time. A small value will yield
  // more accurate solutions at the expense of a slower convergence. iterations
  ...
  cv::UMat flowMat;
  cv::Ptr<cv::optflow::DualTVL1OpticalFlow> dualtlv1 =
